...
\begin{document}
	...
	The first data structure for one image has a 3D structure, $1200 \times 900 \times 26$. Since there are 26 resulting images from one original x-ray image, the dimensionality reduction technique chosen is Principal Component Analysis (PCA) since this is linearly structured whereby only one data element is accessed as a time. PCA is applied to determine which of the 26 images holds crucial information for the detection of bone fracture, whilst at the same time mapping the data to a lower-dimension. The first step to mapping the data into a lower-dimension is adopting the PCA compression method, whereby all 26 images are compressed. The initial step is to determine the covariance matrix for each image. The covariance matrix is then used to determine the principal components. The number of principal components is proportional to the number of variables, $p$ within an image. The resultant principal components, $Z_p$ are computed by determining the eigenvalues of the covariance matrix. The covariance matrix is in a symmetrical form. The eigenvalues found in the covariance matrix are the variances of the principal components. Since the number of principal component are proportional to the number of variables within an image, this means that there are $p$ eigenvalues. All eigenvalues, $\lambda$ are greater than or equal to zero. The largest eigenvalue corresponds to the first principal component. This applies to the next eigenvalue, until the i-th principal component. Therefore $\lambda_i$ corresponds to the i-th eigenvector. Thus the PCA steps applied to each image are as follows: 
		
	The input for the two-step system are two dimensional (2D) images. The structure of the 2D x-ray images is a linear structure. By definition a linear structure is a sequence of data elements, which only one data element is directly evaluated at a time. Therefore the chosen dimensionality reduction technique is Principal Component Analysis (PCA). 
	With this approach, each pixel in the image is evaluated. The size of the images are $ 809 \times 899$ pixels. The objective of the dimensionality reduction technique is to reduce the dimensions of the x-ray image by mapping it onto a lower dimension. The mapping is done by determining as much variation within the data as possible.
